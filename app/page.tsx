      ...
      <main className="cs2-workspace">
        ...
        <section className={`cs2-page cs12-view cs12-view-${view}`} key={view}>
          ...
          {view === "settings" && <>
            ...
            <div className="cs2-subscription-card"><div><span>{isAdmin ? "COMPTE PROPRIÉTAIRE · ACCÈS COMPLET" : complimentaryAccess ? "ACCÈS OFFERT PAR L’ADMINISTRATEUR" : userId ? "ACCÈS BÊTA GRATUIT" : "APERÇU INTERACTIF"}</span><h2>{isAdmin || complimentaryAccess ? "Plan Agency" : "Bêta ClipScale"}</h2><p>{isAdmin || complimentaryAccess ? "Toutes les fonctionnalités Agency sont débloquées sur ce compte." : "Le moteur est en cours de validation. Aucun abonnement payant n’est actif sur ce compte."}</p></div><div><b>{isAdmin || complimentaryAccess ? "Complet" : isDemo ? "Gratuit" : "0 €"} <small>{isAdmin ? "compte propriétaire" : complimentaryAccess ? "accès offert" : isDemo ? "aperçu" : "pendant la bêta"}</small></b><button onClick={exit}>Voir le site public</button></div></div>
